...
function change_year(){
    ...
    document.getElementById('bank_balance_year').value = year;
    changed_balance_year('bank_balance');
    document.getElementById('petty_cash_year').value = year;
    changed_balance_year('petty_cash');
}
...
//Function is called when a year for a specific balance is changed in the balances section
function changed_balance_year(type){
    if(type == 'bank_balance' || type == 'petty_cash'){
        const year = document.getElementById(`${type}_year`);
        const d = new Date();
        if(d.getMonth() >= 3 && year.value >= d.getFullYear()){
            year.value = d.getFullYear();
        } else if(year.value < 2023){
        ...
        const errorText = document.getElementById(`${type}_form_error`);
        errorText.style.display = 'none';
        const xhr = new XMLHttpRequest();
        xhr.open('POST', './inc/get_balance.inc.php');
        xhr.setRequestHeader("Content-Type", 'application/x-www-form-urlencoded');
        xhr.onreadystatechange = function(){
            if(this.status === 200){
                if(this.readyState === 4){
                    ...
                    if(text['error']){
                        ...
                        document.getElementById(`${type}_number`).value = text['data'];
                    } else {
                        errorText.innerText = 'Loading error';
                        errorText.style.color = 'red';
                        errorText.style.display = 'block';
        ...
        switch(type){
            case 'bank_balance':
                xhr.send(`type=Bank Balance&year=${year.value}`);
                break;
            case 'petty_cash':
                xhr.send(`type=Petty Cash&year=${year.value}`);
                break;
        }
    }
}
//Event is called when the bank balance form is submitted
document.getElementById("bank_balance_form").addEventListener("submit", (e)=>{
    e.preventDefault();
    submit_balance_form('bank_balance');
});
//Event is called when the petty cash balance form is submitted
document.getElementById("petty_cash_form").addEventListener("submit", (e)=>{
    e.preventDefault();
    submit_balance_form('petty_cash');
});
//Function is called to submit data for a balance form
function submit_balance_form(type){
    if(type == "bank_balance" || type == "petty_cash"){
        const year = document.getElementById(`${type}_year`);
        const d = new Date();
        if(d.getMonth() >= 3 && year.value >= d.getFullYear()){
        ...
        errorText.style.display = 'none';
        const xhr = new XMLHttpRequest();
        xhr.open('POST', './inc/add_balance.inc.php');
        xhr.setRequestHeader("Content-Type", 'application/x-www-form-urlencoded');
        xhr.onreadystatechange = function(){
            if(this.status === 200){
                if(this.readyState === 4){
        ...
        switch(type){
            case 'bank_balance':
                xhr.send(`type=Bank Balance&year=${year.value}&balance=${document.getElementById(type+'_number').value}`);
                break;
            case 'petty_cash':
                xhr.send(`type=Petty Cash&year=${year.value}&balance=${document.getElementById(type+'_number').value}`);
                break;
        }
    }
}
